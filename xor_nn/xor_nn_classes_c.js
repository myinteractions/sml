...
class WeightTextHandlerTrainerWithAlgorithm extends WeightTextHandler {
	constructor(context) {
		super(context);
	}
...
class NeuronTextHandlerTrainerWithAlgorithm extends NeuronTextHandler {
	constructor(context) {
		super(context);
	}
...
class XORWeightTrainerWithAlgorithm extends Weight {
	constructor(context) {
		super(context);
		this.textHandler = new WeightTextHandlerTrainerWithAlgorithm(context);
	}
...
class XORNeuronTrainerWithAlgorithm extends Neuron {
	constructor(context, iIndex) {
		super(context, iIndex);
		this.textHandler = new NeuronTextHandlerTrainerWithAlgorithm(context);
	}

	initializeWeights(numWeights) {
		this.incomingWeights = new Array(numWeights);
		for(var i=0; i < numWeights; i++) {
			this.incomingWeights[i] = new XORWeightTrainerWithAlgorithm(this.context);
...
class XORNeuronLayerTrainerWithAlgorithm extends NeuronLayer {
	constructor(context, numNeuronsInLayer) {
		super(context, numNeuronsInLayer);
		for(var i=0; i < this.neurons.length; i++) {
			this.neurons[i] = new XORNeuronTrainerWithAlgorithm(this.context, i);
		}
	}
...
class XORNeuralNetworkTrainerWithAlgorithm extends XORNeuralNetworkTrainer {
	constructor(context) {
		super(context);
	}
	...
	initializeNeuronLayers() {
		...
		for(var i=0; i < this.numberOfLayers; i++) {
			this.neuronLayers[i] = new XORNeuronLayerTrainerWithAlgorithm(this.context, neuronsInEachLayer[i]);
		}
	...
	drawNeuralNetwork() {
		...
		// this.textHandler.handleNeuralNetworkText(TOP_NEURON_LOCATIONS_FOR_ALGORITHM);
		this.fillXOROutputDataContainer();
		this.textHandler.drawXORErrorOutputText(TOP_NEURON_LOCATIONS_FOR_ALGORITHM, this.xorOutputDataContainer);
